...
export class UsuariosService {
  ...
  async crearUsuario_ahbb(datos_ahbb: any) {
    ...
    const nuevoUsuario_ahbb = await this.prisma_ahbb.td_usuario_ahbb.create({
      data: {
        ...
        estadoCuenta_ahbb:
          datos_ahbb.estadoCuenta ?? (rolNormalizado_ahbb === 'ALUMNO' ? 'PENDIENTE_APROBACION' : 'ACTIVO'),
  ...
  async actualizarContrasena_ahbb(id_usuario_ahbb: number, hashContrasena_ahbb: string) {
  ...
  async actualizarEstadoCuenta_ahbb(id_usuario_ahbb: number, estadoCuenta_ahbb: string) {
    const usuarioActualizado_ahbb = await this.prisma_ahbb.td_usuario_ahbb.update({
      where: { id_usuario_ahbb },
      data: {
  ...
  async guardarFirmaDigital_ahbb(id_usuario_ahbb: number, imagenBase64_ahbb: string) {
    const usuario_ahbb = await this.prisma_ahbb.td_usuario_ahbb.findUnique({
      where: { id_usuario_ahbb },
    });
    ...
    await writeFile(rutaFisica_ahbb, Buffer.from(coincidencia_ahbb[1], 'base64'));
