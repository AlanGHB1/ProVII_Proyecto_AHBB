/**
 * autenticacionServicio_ahbb.ts — Servicio de autenticación
...
// ─── Respuestas tipadas del servicio ──────────────────
export interface IRespuestaAuth_ahbb {
  exito: boolean;
  usuario: IUsuario | null;
  mensaje: string;
  token?: string; // Solo cuando hay API real
}
 ...
 */
export const obtenerUsuarios_ahbb = async (): Promise<IUsuario[]> => {
  if (MODO_API_AHBB) {
    const respuesta_ahbb = await apiCliente_ahbb.get<IUsuario[]>('/usuarios');
    return respuesta_ahbb.data;
  }

  // Modo LocalStorage
  return obtenerDato_ahbb<IUsuario[]>(CLAVES_STORAGE_AHBB.USUARIOS, []);
};
 ...
 */
export const guardarUsuarios_ahbb = (usuarios_ahbb: IUsuario[]): void => {
  if (!MODO_API_AHBB) {
 ...
 */
export const iniciarSesion_ahbb = async (
  correo_ahbb: string,
  contrasena_ahbb: string,
): Promise<IRespuestaAuth_ahbb> => {
  if (MODO_API_AHBB) {
    try {
  ...
  // Modo LocalStorage
  const usuarios_ahbb = obtenerDato_ahbb<IUsuario[]>(CLAVES_STORAGE_AHBB.USUARIOS, []);
  ...
  // Guardar sesión
  guardarDato_ahbb<IClaveSesion>(CLAVES_STORAGE_AHBB.SESION, { id: usuario_ahbb.id });
 ...
 */
export const registrarUsuario_ahbb = async (
  datosUsuario_ahbb: IRegistroUsuario,
): Promise<IRespuestaAuth_ahbb> => {
  if (MODO_API_AHBB) {
    try {
 ...
 */
export const cerrarSesion_ahbb = async (): Promise<void> => {
  if (MODO_API_AHBB) {
    try {
 ...
 */
export const recuperarSesion_ahbb = async (): Promise<IUsuario | null> => {
  if (MODO_API_AHBB) {
    try {
      const respuesta_ahbb = await apiCliente_ahbb.get<IUsuario>('/auth/perfil');
      return respuesta_ahbb.data;
    } catch {
      return null;
    }
  ...
  // Modo LocalStorage
  const sesion_ahbb = obtenerDato_ahbb<IClaveSesion | null>(CLAVES_STORAGE_AHBB.SESION, null);
