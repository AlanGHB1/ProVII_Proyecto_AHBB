/**
 * cursosServicio_ahbb.js — Servicio de cursos
...
const leerCursosLS_ahbb = () =>
  obtenerDato_ahbb(CLAVES_STORAGE_AHBB.CURSOS, []);

/** Guarda la lista de cursos en LocalStorage */
const guardarCursosLS_ahbb = (cursos_ahbb) =>
  guardarDato_ahbb(CLAVES_STORAGE_AHBB.CURSOS, cursos_ahbb);
...
export const crearCurso_ahbb = async (datosCurso_ahbb) => {
  if (MODO_API_AHBB) {
    const respuesta_ahbb = await apiCliente_ahbb.post('/cursos', datosCurso_ahbb);
    return respuesta_ahbb.data;
  }
...
export const actualizarCurso_ahbb = async (id_ahbb, datosCurso_ahbb) => {
  ...
  cursos_ahbb[indice_ahbb] = { ...cursos_ahbb[indice_ahbb], ...datosCurso_ahbb };
  guardarCursosLS_ahbb(cursos_ahbb);
  return true;
};
 ...
 */
export const inicializarCursos_ahbb = (cursosIniciales_ahbb) => {
  if (MODO_API_AHBB) return []; // En modo API, los cursos vienen del servidor

  const cursosExistentes_ahbb = obtenerDato_ahbb(
