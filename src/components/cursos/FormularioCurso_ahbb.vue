...
import type { ICurso, ICursoFormulario, IOpcionSelect, DiaSemana, EstatusCurso } from '../../types';
import { useCursosStore_ahbb } from '../../stores/cursosStore_ahbb';

const props = defineProps<{
  cursoInicial_ahbb?: ICurso | null;
  esEdicion_ahbb?: boolean;
}>();

const emit = defineEmits<{
  (e: 'guardar', datos: ICursoFormulario): void;
  (e: 'cancelar'): void;
}>();

const cursosStore_ahbb = useCursosStore_ahbb();

const diasDisponibles_ahbb: IOpcionSelect[] = [
...
const opcionesEstatus_ahbb: IOpcionSelect[] = [
  { label: 'Pendiente', value: 'pendiente' },
  { label: 'Activo', value: 'activo' },
  { label: 'Inactivo', value: 'inactivo' },
];

const formulario_ahbb = ref<ICursoFormulario>({
  nombre: '',
  descripcion: '',
  profesor: '',
  duracionHoras: 20,
...
const opcionesPrelacion_ahbb = computed<IOpcionSelect[]>(() =>
...
onMounted((): void => {
  if (props.cursoInicial_ahbb) {
    formulario_ahbb.value = {
      ...
      dias: [...(props.cursoInicial_ahbb.dias || [])] as DiaSemana[],
      ...
      estatus: (props.cursoInicial_ahbb.estatus || 'pendiente') as EstatusCurso,
      temario: props.cursoInicial_ahbb.temario || '',
    };
  }
});

const enviarFormulario_ahbb = (): void => {
