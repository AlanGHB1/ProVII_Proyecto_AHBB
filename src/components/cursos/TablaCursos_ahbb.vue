...
const columnas_ahbb = [
  ...
  { name: "dias", label: "Dias", field: "dias", align: "left",
    format: (val) => val?.map((d) => d.charAt(0).toUpperCase() + d.slice(1, 3)).join(", ") || "—" },
  { name: "horario", label: "Horario", align: "left",
    field: (row) => `${row.horaInicio} - ${row.horaFin}` },
  { name: "inscritos", label: "Inscritos", align: "center",
...
const colorEstatus_ahbb = (estatus) => {
  const mapa = { activo: "positive", pendiente: "warning", inactivo: "negative" };
