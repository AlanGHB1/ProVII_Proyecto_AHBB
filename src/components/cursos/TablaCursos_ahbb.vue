...
import type { ICurso, EstatusCurso } from '../../types';
import { useCursosStore_ahbb } from '../../stores/cursosStore_ahbb';

const cursosStore_ahbb = useCursosStore_ahbb();
const emit = defineEmits<{
  (e: 'eliminar', curso: ICurso): void;
}>();

// Definición de columnas para QTable
const columnas_ahbb = [
  { name: 'nombre', label: 'Curso', field: 'nombre', align: 'left' as const, sortable: true },
  { name: 'profesor', label: 'Profesor', field: 'profesor', align: 'left' as const, sortable: true },
  {
    name: 'dias',
    label: 'Dias',
    field: 'dias',
  ...
  },
  {
    ...
    label: 'Horario',
    align: 'left' as const,
    field: (row: ICurso) => `${row.horaInicio} - ${row.horaFin}`,
  },
  {
    name: 'inscritos',
    label: 'Inscritos',
    align: 'center' as const,
    field: (row: ICurso) => `${row.estudiantesInscritos}/${row.topeEstudiantes}`,
  },
  { name: 'estatus', label: 'Estatus', field: 'estatus', align: 'center' as const, sortable: true },
  { name: 'acciones', label: 'Acciones', align: 'center' as const, field: '' },
];

const colorEstatus_ahbb = (estatus: EstatusCurso): string => {
  const mapa: Record<EstatusCurso, string> = {
    activo: 'positive',
    pendiente: 'warning',
    inactivo: 'negative',
  };
